...
class PartitionsTest {
    ...
    @Test
    void testShortTail() {
        Partitions partitions = Partitionings.LONG.create(3, 80).of(2000);
        assertSizes(partitions, 640L, 640L, 640L, 80L);
    }

    @Test
    void testShortTailAligned() {
        Partitions partitions = Partitionings.LONG.create(3, 80).of(1996);
        assertSizes(partitions, 632L, 640L, 640L, 84L);
    }

    @Test
    void test692() {
        Partitions partitions = Partitionings.LONG.create(6).of(104);
        assertSizes(partitions, 16L, 16L, 16L, 16L, 16L, 24L);
    }

    @Test
    void testLongAligned() {
        Partitions partitions = Partitionings.LONG.create(3).of(65);
        assertSizes(partitions, 16L, 24L, 25L);
    }

    @Test
    void testLongAlignedShort() {
        Partitions partitions = Partitionings.LONG.create(3).of(52);
    ...
    void fragment() {
        ...
        Partitions partitions = Partitionings.LONG.create(cpus, 250)
